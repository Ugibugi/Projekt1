...
public:
	...
	template<typename T1,typename T2>
	void addHandler(uint32_t groupId1, uint32_t groupId2, std::function<void(T1*,T2*)> fun)
	{
		...
		static_assert(std::type_index(typeid(T1)) == typeMap.at(groupId1), "Type 1 does not match type assigned to Group 1");
		static_assert(std::type_index(typeid(T2)) == typeMap.at(groupId1), "Type 2 does not match type assigned to Group 2");
